...
def estimate_distance(lms):
    ...
    if shoulder_width < 0.01: return 10.0
    return min(CAMERA_FOCAL_FACTOR / shoulder_width, 10.0)
...
# 🎯 ฟังก์ชันสุ่มเป้าหมายใหม่สำหรับ โหมด 1 โดยเฉพาะ (ปรับลดระยะให้ใกล้แขนขึ้นอีก โน้มตัวแค่เบาๆ)
def spawn_target_mode1(w, h, user_cx, user_cy, arm_span, existing_targets=None):
    min_dist_sq = 0.15 ** 2
    arm_span = max(0.35, arm_span)

    for _ in range(100):
        # สุ่มโซนให้มีโอกาสขึ้น ตรงกลางซ้าย, ตรงกลางขวา, หรือ ด้านบน
...
# 🎯 ฟังก์ชันสุ่มเป้าหมายเดิมสำหรับโหมด 3 (ไม่มีการเปลี่ยนแปลง)
    ...
    while True:
        ...
        if results.pose_landmarks:
            ...
            if mode == 1:
                ...
                # 2. กางแขนออกเพื่อวัดระยะรัศมีเป้าหมาย
                elif leaning_state == 2:
        ...
        # 📌 แจ้งเตือนกลางหน้าจอ โหมด 1 และ 3
        if mode == 1:
            if leaning_state == 1:
                cv2.rectangle(frame, (w // 2 - 200, 30), (w // 2 + 200, 100), COLOR_BG, -1)
                draw_text_bold(frame, "ยืนนิ่งๆ เพื่อเซ็ตศูนย์กลาง...", (w // 2 - 150, 50), 0.8, COLOR_AMBER)
            elif leaning_state == 2:
                cv2.rectangle(frame, (w // 2 - 250, 30), (w // 2 + 250, 100), COLOR_BG, -1)
                if ready_start_t is not None:
                    rem_hold = max(0.0, 1.0 - (now - ready_start_t))
                    draw_text_bold(frame, f"ค้างไว้... {rem_hold:.1f} วิ", (w // 2 - 120, 50), 0.8, COLOR_GREEN)
                else:
                    draw_text_bold(frame, "กางแขนออกด้านข้าง (T-Pose)", (w // 2 - 220, 50), 0.8, COLOR_ORANGE)
            elif leaning_state == 3:
        ...
        # 🎯 ตรวจสอบว่ากำลัง "ทดสอบอยู่" หรือไม่ เพื่อสั่งซ่อนกรอบ UI หลัก (ให้หน้าจอโล่ง)
        is_playing = False
        ...
        if not is_playing:
            # แสดง UI หลักเฉพาะตอน ยังไม่เริ่ม หรือ ทดสอบเสร็จแล้ว
