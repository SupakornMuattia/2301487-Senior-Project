...
# ถ้าระยะในแอป "ไกลกว่า" ความจริง -> ให้ ลด ตัวเลขลง
# ถ้าระยะในแอป "ใกล้กว่า" ความจริง -> ให้ เพิ่ม ตัวเลขขึ้น
# ─────────────────────────────────────────────
...
TARGET_DISTANCE_MIN = 4.5  # ระยะที่ต้องการให้อยู่ในโซนสีเขียว (เมตร)
...
def estimate_distance(lms):
    """คำนวณระยะห่าง (เมตร) โดยใช้ความกว้างของหัวไหล่เป็นเกณฑ์ และค่า Focal Factor"""
    ...
    while True:
        ...
        if results.pose_landmarks:
            ...
            if mode == 1:
                if leaning_state == 1:
                    ...
                    if not is_cooldown: draw_reach_targets(frame, ar_targets, w, h)
            ...
            # --- MODE 2 ---
                ...
                if gait_state == 1:
                    ...
                    if current_distance_m < TARGET_DISTANCE_MIN:
                        ...
                        # ถึงระยะที่เหมาะสม (สีเขียว) เปลี่ยนสถานะอัตโนมัติ ไม่ต้องกด Space ซ้ำ
            ...
            # --- MODE 3 ---
                ...
                if m3_state == 1:
                    ...
                    if not is_cooldown: draw_reach_targets(frame, m3_targets, w, h)
                    for lm_id in [mp_pose.PoseLandmark.LEFT_WRIST, mp_pose.PoseLandmark.RIGHT_WRIST]:
                        px, py = lm_px(lms[lm_id], w, h);
                        cv2.circle(frame, (px, py), 12, COLOR_CORAL, -1)
        ...
        if key in (ord('q'), 27):
            ...
            if mode == 1:
                ...
                # แก้ไขการทำงานของ Spacebar ในโหมด 2
                if gait_state == 0 or gait_state == 4:
                    # กดครั้งเดียวเพื่อเข้าสู่โหมดจัดระยะ
                    gait_state = 1
                elif gait_state == 3:
                    # กดเพื่อหยุดแมนนวลได้ถ้าระหว่างเดินเกิดปัญหา
                    gait_state = 4
